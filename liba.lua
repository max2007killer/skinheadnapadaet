...
function finity.new(theme, gprojectName, thinProject)
    local finityObject = {}
    local self2 = finityObject
    local self = finity

    if not finity.gs["RunService"]:IsStudio() and self.gs["CoreGui"]:FindFirstChild("minted") then
        self.gs["CoreGui"]:FindFirstChild("minted"):Destroy()
    end

    local projectName = false
    local thinMenu = false
    
    if theme == "dark" then theme = finity.dark_theme elseif theme == "light" then theme = finity.theme end
    ...
    function self2:Category(name)
        ...
        function category:Sector(name, side)
        local sector = {}
        side = side
        ...
        local function calculateSector()
            ...
            function sector:sectorvisible(value)
                sector.frame.Visible = value
            end
            ...
            function sector:Cheat(kind, name, callback, data)
                ...
                function cheat:visible(value)
                    cheat.frame.Visible = value
                end
                ...
                if kind then
                    ...
                    elseif string.lower(kind) == "textbox" then
                        ...
                        local textboxtext = data and data.default
